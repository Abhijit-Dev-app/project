...
// ── Helper: Sync alert status when ticket status changes ──
const syncAlertStatus = (ticketId, ticketStatus) => {
    // Map ticket status → alert status
    const alertStatusMap = {
        'OPEN'        : 'OPEN',
        'IN_PROGRESS' : 'IN_PROGRESS',
        'PENDING'     : 'IN_PROGRESS',  // alerts have no PENDING
        'RESOLVED'    : 'RESOLVED',
        'CLOSED'      : 'CLOSED'
    };
    const alertStatus = alertStatusMap[ticketStatus];

    // Get alert_id from ticket then update alert
    db.query('SELECT alert_id FROM tickets WHERE id = ?', [ticketId], (err, tickets) => {
        if (err) return console.error('[Ticket] Sync lookup error:', err.message);
        if (!tickets.length || !tickets[0].alert_id) return; // no linked alert

        const alertId = tickets[0].alert_id;
        db.query(
            'UPDATE alerts SET status = ? WHERE id = ?',
            [alertStatus, alertId],
            (err) => {
                if (err) console.error('[Ticket] Alert sync error:', err.message);
                else console.log(`✅ Alert #${alertId} synced to ${alertStatus} from ticket #${ticketId}`);
            }
        );
    });
};
...
router.post('/', verifyToken, (req, res) => {
    ...
    generateTicketRef((ticket_ref) => {
        ...
        db.query(sql, [
            ...
            // Sync alert status to OPEN when ticket is created
            if (alert_id) {
                db.query('UPDATE alerts SET status = ? WHERE id = ?', ['OPEN', alert_id], (err) => {
                    if (err) console.error('[Ticket] Alert sync on create error:', err.message);
                    else console.log(`✅ Alert #${alert_id} synced to OPEN on ticket creation`);
                });
            }
...
// ─────────────────────────────────────────────────────────
// PATCH /api/tickets/:id/status — Update ticket status + sync alert
// ─────────────────────────────────────────────────────────
router.patch('/:id/status', verifyToken, (req, res) => {
    ...
    const resolved_at = newStatus === 'RESOLVED' ? new Date() : null;
    const sql         = resolved_at
        ? 'UPDATE tickets SET status = ?, resolved_at = ? WHERE id = ?'
        : 'UPDATE tickets SET status = ? WHERE id = ?';
    const params      = resolved_at
        ? [newStatus, resolved_at, id]
        : [newStatus, id];
    ...
    db.query(sql, params, (err, result) => {
        ...
        // Log to timeline
        logTimeline(id, 'STATUS_CHANGE', `Status changed to ${newStatus} by ${req.user.username}`, req.user.username);

        // ✅ Sync linked alert status automatically
        syncAlertStatus(id, newStatus);

        res.json({ status: 'Updated', newStatus });
    });
});
